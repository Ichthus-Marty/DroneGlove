...
//PI variabales
float Cp = 0.05; //Constant for the P
...
//Drone variables
float Power[4] = {0.0,0.0,0.0,0.0}; // Array for the data output per engine.
    ...
    while (imu->IMURead()) {// get the latest data if ready yet
      if(Radio.ReceiveDecode(&pRI)){
        ...
        if ((delta = now - lastRate) >= 1000) {
          ...
          /*
          if(abs(roll-rollPrev)>= 10 && runFlag){
            return;
          }
          */
          ...
          if(abs(pitch-pitchPrev)>=10 && runFlag){
            return;
          }
          */
          pitchPrev = pitch;
